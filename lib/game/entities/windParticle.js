...
.defines(function(){
	...
	EntityWindParticle = ig.Entity.extend({
		...
		init: function( x, y, settings ){
			...
			this.windVector = ig.game.getEntitiesByType( EntityWindVector )[0];
		...
		reset: function( x, y, settings ) {
			...
			this.idleTimer = new ig.Timer();
			//add animations
			this.addAnim( 'ripple', 0.08, [0,1,2,3,4,5,6,7] );
			this.currentAnim = this.anims.ripple;
			this.windVector = ig.game.getEntitiesByType( EntityWindVector )[0];
    	}

	});
ig.EntityPool.enableFor( EntityWindParticle );
});
